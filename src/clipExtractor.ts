...
export const extractVideoIdAndClipTimes = async (
  clipUrl: string
): Promise<{ videoId: string; startTime: number; endTime: number } | null> => {
  ...
  try {
    ...
    scriptTags.each((i, script) => {
      ...
      if (scriptContent && scriptContent.includes('"clipConfig"')) {
        const videoIdMatch = scriptContent.match(
          /"videoId":"([a-zA-Z0-9_-]{11})"/
        );
        const clipConfigMatch = scriptContent.match(
          /"clipConfig":\{"postId":"([a-zA-Z0-9_-]+)","startTimeMs":"(\d+)","endTimeMs":"(\d+)"\}/
        );
        ...
        if (videoIdMatch && clipConfigMatch) {
          ...
          console.log(
            `Extracted videoId: ${videoId}, startTime: ${startTime}, endTime: ${endTime}`
          );
        }
      }
    });
    ...
    console.error(
      `Error al extraer el ID del video y tiempos del clip: ${error}`
    );
    if (error.response) {
