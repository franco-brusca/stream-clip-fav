...
export const downloadAndProcessVideo = async (
  videoUrl: string,
  clipInfo: { videoId: string; startTime: number; endTime: number },
  videoQuality: string,
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const clippedVideoTempFilePath = path.join(
      os.tmpdir(),
      `${uuidv4()}-video.mp4`
    );
    const clippedAudioTempFilePath = path.join(
      os.tmpdir(),
      `${uuidv4()}-audio.mp4`
    );
    ...
    const downloadAndClipVideo = new Promise<void>((resolve, reject) => {
      ffmpeg(format.url)
        ...
        .on("progress", (progress) => {
          if (progress.percent)
            sendProgress(
              `{"progress": "${(progress.percent * 10).toFixed(2)}%"}`
            );
        })
        .on("end", () => {
    ...
    ffmpeg()
      ...
      .on("end", () => {
        ...
        sendProgress(
          `${JSON.stringify({ message: "File ready", file: outputFilePath })}`
        );
      })
      .on("error", (err, stdout, stderr) => {
    ...
    res.write(
      `data: ${JSON.stringify({
        error: "Error en downloadAndProcessVideo",
      })}\n\n`
    );
    res.end();
  }
